...
//(e) => changeHoverState(e)
  return (
    <div id="Masters" className="tabcontent">
      <div className="tabcontent-title" onMouseOver={(e) => changeHoverState(e)} onMouseLeave={(e) => changeHoverState(e)} onClick={() => setContentsHidden(!contentsHidden)}>
        <h2>{props.title}</h2>
        <img className="menu-control" src={expandMenu} style={contentsHidden ? {transform:"rotate(0deg)"}:{transform:"rotate(180deg)"}}></img>
      </div>
      <div className="tabcontent-content" id="masters-content" style={contentsHidden ? {display:"none"} : {display:""}}>
        <ul className="portfolio-list">
          {portfolio.map((subject, key) => (
            <li key={key} onClick={() => props.PDFReader(subject.document)}>{subject.title}</li>
          ))}
